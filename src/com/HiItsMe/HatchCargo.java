...
class HatchCargo {
	...
	private double[] hatchState = {0, 0, 0};
	int hs = 0;
	private double[] cargoState = {0, 0, 0};
	int cs = 0;
	...
	void setState(boolean hatch) {
		if(hatch) { hs++; hs%=4; } else { cs++; cs%=4; }
		switch(hatch ? hs : cs) {
			case 0:
				if(hatch) hatchState = new double[]{0, 0, 0}; else cargoState = new double[]{0, 0, 0};
				break;
			case 1:
				if(hatch) hatchState = new double[]{0, 0, 1}; else cargoState = new double[]{0, 0, 1};
				break;
			case 2:
				if(hatch) hatchState = new double[]{0, 1, 0}; else cargoState = new double[]{0, 1, 0};
				break;
			case 3:
				if(hatch) hatchState = new double[]{1, 0, 0}; else cargoState = new double[]{1, 0, 0};
				break;
		}
	}
	void set(double[][] dat) {
